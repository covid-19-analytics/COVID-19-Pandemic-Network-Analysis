...
# *************************************************************************
# [_02b_reproduce_final_results_by_binarized_vF.R]
# 
#   Aim: Produce network statistics, risk scores, risk contribution 
#     by binarized matrix (r= 0.4, 0.5, 0.6),
#     and different imputation cutoff (0.5, 0.6, 0.7, 0.8)
# *************************************************************************
...
#   Extract the required network statistics (Actual, Simulated) -----
...
actual_date_begin <- ymd("2020-01-22") # our analysis period: from Jan 22
...
# this will be ignored, if using rolling_window, but KEEP this ****
...
basic_setup <- list(
  ...
  "node_type"= node_type, # *** Remarks: havent test ALL ****
...
load_historical_data <- function(tmp_setup) {
  ...
  region_meta <- readr::read_csv(
    here::here("raw_data", tmp_setup$target_data[2])) %>%
  ...
  df_newCase <- readr::read_csv(
    here::here("raw_data", tmp_setup$target_data[1]), na= c("", "NA"))
...
calc_riskScore <- function(
  ...
  lapply(seq(length(l_result)), function(i) {
    ...
    # sub_names <- colnames(tmp_corr) 
    # A[sub_names, sub_names] <- tmp_corr
    
    # *** Remarks: Fixing the issue ***
    ...
    if (weight_type=="mixed_active") {
      ...
      if (need_standardize==TRUE) {
        v_contribution <- v_contribution / k_S
      } else {}
...
create_graph <- function(tmp_setup, l_result, region_meta,
  ...
  lapply(seq(length(l_result)), function(i) {
    ...
    if (threshold_option=="r") {
      tmp_edges <- res.melted %>% dplyr::filter(corr > tmp_threshold) # **************** applied the correlation threshold
    } else {}
...
df_deathCase <- read_csv(
  here::here("raw_data", sprintf("upto_%s_FINAL_deaths_World.csv", date_str))) %>%
...
l_all_network_stat <- lapply(seq(length(l_obj_graph)), function(idx) {
  obj_graph <- l_obj_graph[[idx]]
  
  lapply(seq(length(obj_graph$network)), function(i) {
    
    tmp_network <- obj_graph$network[[i]]
    g <- tmp_network$graph
    
    tmp_network$num_edge <- gsize(g)
    tmp_network$edge_density <- edge_density(g, loops= FALSE)
    tmp_network$CC_global <- transitivity(g, type= "global")
    tmp_network$assortativity <- assortativity_degree(g, directed= FALSE)
  ...
  lapply(l_network, function(net) {
    tmp_df <- data.frame(
      period= net$period, 
      num_edge= net$num_edge,
      edge_density= net$edge_density,
      CC_global= net$CC_global,
      assortativity= net$assortativity
    )
    
    return(tmp_df)
  }) -> l_network_stat
  
  df_network_stat <- do.call("rbind", l_network_stat)
  
  df_network_stat$threshold_val <- obj_graph$threshold_val
  df_network_stat$threshold_type <- obj_graph$threshold_type
  
  return(df_network_stat)
  
})

df_final_network_stat <- do.call("rbind", l_all_network_stat)

# *** Remarks: This code is for converting rolling period number into calendar day ***
df_final_network_stat <- df_date %>%
  dplyr::group_by(period_no) %>%
  dplyr::filter(row_number()==n()) %>%
  dplyr::select(date, period_no) %>%
  left_join(df_final_network_stat, ., by= c("period"= "period_no"))
...
df_WHO_region <- read_csv(
  here::here("raw_data", "WHO_Regions.csv"))
...
if (length(l_risk_score) > 0) {
  ...
  for (i in seq(length(l_risk_score))) {
    ...
    if(obj_risk_score$weight_type %ni% c("mixed", "mixed_active")) { next() }
    if(unique(tmp_risk_score$need_standardize)!=FALSE) { next() }
    # if(unique(tmp_risk_score$need_standardize)!=TRUE) { next() }
    ...
    tmp_plot <- ggplot(
      tmp_risk_score, aes(
        date, s, 
        linetype= factor(threshold_type),
        color= factor(threshold_val),
        shape= factor(threshold_type))) +
      ...
      labs(title= sprintf("Time series - Risk score (%s) - (with %s weights)", 
                          "World", obj_risk_score$weight_type),
           x= ifelse(basic_setup$sampling_type=="rolling" |
                       basic_setup$sampling_type=="rolling_window",
                     "rolling period no", "period no")) +
...
df_final_network_stat %>% 
  dplyr::select(date, num_edge, edge_density, CC_global, assortativity, threshold_val, threshold_type) %>%
  write_csv(., here::here(
    proj_name, "output", 
    sprintf("df_final_network_stat_%s_%s%s_Rit_hat_%s.csv", date_str, "r", 
...
df_ts_St %>%
  ...
  write_csv(., here::here(
    proj_name, "output", 
    sprintf("df_final_riskScore_%s_%s%s_Rit_hat_%s.csv", date_str, "r", 
            gsub("[^0-9]+", "", threshold_val),
            cutoff_I)))
...
df_ts_risk_contrb %>%
  ...
  write_csv(., here::here(
    proj_name, "output",
    sprintf("df_final_riskContrb_%s_%s%s_by_binary_Rit_hat_%s.csv", date_str, "r",
            gsub("[^0-9]+", "", threshold_val), cutoff_I)))

df_ts_risk_contrb_sp %>%
  write_csv(., here::here(
    proj_name, "output",
    sprintf("df_final_riskContrb_detail_%s_%s%s_by_binary_Rit_hat_%s.csv", date_str, "r",
            gsub("[^0-9]+", "", threshold_val), cutoff_I)))
